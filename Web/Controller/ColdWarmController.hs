module Web.Controller.ColdWarmController where
...
instance Controller ColdWarmController where
  action APICandlesAction = do
    logController Info $ "[ColdWarmController][CandlesAction]" <> requestUrl
  ...
  action APISymbolsAction = do
    logController Info $ "[ColdWarmController][SymbolsAction]" <> requestUrl
