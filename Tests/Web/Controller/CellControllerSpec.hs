...
module Tests.Web.Controller.CellControllerSpec (tests) where
...
tests = aroundAll (withIHPApp WebApplication config) do
  describe "CellController (integration)" do
    ...
    it "CellReadAction creates first cell when owner has no cells" $ withContext do
      ...
      response <- callActionWithParams CellReadAction [("ownerType", "note"), ("ownerId", cs (tshow ownerId))]
      body <- responseBody response
      decodeCellOrders body `shouldBe` Just [1]
      ...
      after <- query @Cell
        ...
        |> fetch
      map (get #cellOrder) after `shouldBe` [1]
